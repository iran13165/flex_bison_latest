...
%token ADA ALL AMMSC ANY AS ASC AUTHORIZATION BETWEEN BY
%token CHARACTER C CHECK CLOSE COBOL COMMIT CONTINUE CREATE CURRENT
%token CURSOR DECIMAL DECLARE DEFAULT DELETE DESC DISTINCT DOUBLE
%token ESCAPE EXISTS FETCH FLOAT FOR FORTRAN FOREIGN FOUND FROM GOTO
%token GRANT GROUP HAVING IN INDICATOR INSERT INTEGER INTO
%token IS KEY LANGUAGE LIKE MODULE NULLX NUMERIC OF ON OPEN OPTION
%token ORDER PARAMETER PASCAL PLI PRECISION PRIMARY PRIVILEGES PROCEDURE
